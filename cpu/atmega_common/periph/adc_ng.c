...
#define AVCC                (1800U)
#elif !defined(AVCC)
/* AVCC can be overwritten in periph_conf.h if these defaults don't match
 * otherwise AVCC is usually tied to VCC, which mostly is 5V for ATmegas
 */
#define AVCC                (5000U)
#endif /* AVCC */
...
static int _single(void *handle, int32_t *dest)
{
    ...
    if (flags & FLAG_NOISE_CANCEL) {
        ...
        while (atmega_is_uart_tx_pending()) {}
