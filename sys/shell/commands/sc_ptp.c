...
static void print_clock_id(const uint8_t *id)
{
...
int sc_ptp(int argc, char **argv)
{
    ...
    if (IS_USED(MODULE_PTP)) {
        printf("%s", "Local Clock ID:           ");
        print_clock_id(ptp_local_clock_id);
        printf("%s", "Selected Server Clock ID: ");
        print_clock_id(ptp_server_clock_id);
        printf("Current offset to UTC time: %d\n", (int)ptp_utc_offset);
        printf("Estimated network delay (whole round trip): %lu\n",
               (unsigned long)ptp_rtt);
    }
    return 0;
}
